# U09　黯黑龍王計時器 `/horntail`

狀態：進行中
進度：19/19（09-17～09-19 是 2026-08-24 使用實況後的修正，等實機驗收）
...
一則公開面板，三顆按鈕分別對應黯黑龍王的三個招式，點下去開始倒數，快歸零時語音提醒，歸零後自動接下一輪，再點一次停止。
...
| 檔案 | 改什麼 |
|---|---|
| `src/main.js` | **已加 `GatewayIntentBits.GuildVoiceStates`**（09-13）。這是領域外的檔案，2026-08-21 經使用者同意才改的 |
...
實機驗收後追加的需求（2026-08-21，見下方「追加需求」）：

- [x] 09-11 伺服器管理員也能操作（`isGmMember()` 改為「GM 身分組 **或** Administrator」）
- [x] 09-12 非 GM／非管理員的斜線指令列表看不到 `/horntail`（程式端已改成 `0`，**伺服器端的覆寫仍需人工加**）
- [x] 09-13 語音頻道沒人時自動收面板（`src/main.js` 已加 `GuildVoiceStates` intent，使用者同意；寬限 60 秒）
- [x] 09-14 只能在**語音頻道的聊天室**使用
- [x] 09-15 面板新增「結束面板」按鈕，可以手動收掉
- [x] 09-16 每一次收面板都寫 log（原本只有「面板建立」有紀錄）

2026-08-24 使用實況後的修正：

- [x] 09-17 倒數改由用戶端算（`<t:秒:R>`），bot 只在狀態改變時編輯
- [x] 09-18 語音提醒改回剩 5 秒
- [x] 09-19 再按一次是「重新計時」而不是「停止」
...
**2026-08-21 使用者同意動 `main.js`，已完成。實際做法比上表簡單：**

09-14 上線後面板一定開在語音頻道的聊天室，所以不必再去猜要綁哪一個頻道——
**面板所在的頻道本身就是那個語音頻道**，直接看 `entry.message.channel.members` 即可
（那是靠 `GuildVoiceStates` 即時算出來的，不是快照）。上表「沒在語音頻道就不綁」那一列不再需要。

| 項目 | 實際做法 |
|---|---|
| 監看對象 | `entry.message.channel`，就是面板所在的語音頻道 |
| 人數 | `channel.members` 濾掉 `user.bot`。只剩 bot 就算沒人 |
| 寬限 | `VOICE_EMPTY_MS = 60 秒`（`src/config/horntail.js`）。設 `0` 就是沒人立刻收 |
| 狀態 | `entry.emptySince`，有人回來就清成 `null` 重新計 |
| 優先度 | 這是**唯一一個「計時器跑著也會收」**的條件——人都走光了，倒數給誰看 |

**`GuildVoiceStates` 是硬前提**：少了它 `channel.members` 永遠是 0 人，
面板會在寬限時間到就被誤收。改動 intent 之後 bot 必須完整重啟才會生效。

### 09-14　只能在語音頻道的聊天室使用（2026-08-21 使用者追加，已完成）

`ctx.channel.isVoiceBased()` 為假就拒絕。語音頻道內建的文字聊天，
**頻道本身就是那個語音頻道**，所以這個判斷同時涵蓋「面板開在語音現場」。

理由不只是整潔：TTS 是由「**正在看該頻道**的人」的用戶端朗讀的。
開在一般文字頻道的話，人在語音、眼睛在別的頻道，提醒等於不存在。

### 09-15　手動結束面板（2026-08-21 使用者追加，已完成）

原本只有「全部停止」，它只把計時器停下、面板留著，**沒有任何方式可以主動收掉面板**——
用完只能等兩小時總時限或 30 分鐘閒置。

新增第二顆按鈕「結束面板」（紅色，`ht:end:<channelId>`），走既有的 `closePanel()`：
停所有計時器、訊息標「面板已結束」、移除所有按鈕。

**兩顆按鈕刻意分開**：打王中途常常要全部暫停一下，如果只有一顆而它會收掉面板，
暫停一次就要重下一次指令。

實作上這一支是唯一**要 `await`** 的按鈕：`closePanel()` 自己會編輯訊息，
不經過 tick 迴圈（面板都已經從 `panels` 移除了，tick 不會再碰它）。
順序是先 `deferUpdate()` 佔住互動（三秒內一定要回應），再去收面板。
...
**09-11／09-12／09-14／09-15 做完後要補驗的（2026-08-21，尚未驗收）**：

12. 沒有 GM 身分組的**伺服器管理員**下 `/horntail` 與點按鈕 → 都可以操作
13. 非 GM／非管理員打 `/` → **指令列表裡沒有 `/horntail`**（伺服器端覆寫加好後，GM 要看得到）
14. **在一般文字頻道**下 `/horntail` → 被拒絕並說明要去語音頻道的聊天室，沒有面板產生
15. **在語音頻道的聊天室**下 `/horntail` → 正常開出面板
16. 面板上有「全部停止」與「結束面板」兩顆按鈕
17. 計時器跑著時按「全部停止」→ 計時全停、**面板還在**、可以再按招式重新開始
18. 按「結束面板」→ 訊息變成「面板已結束」、**所有按鈕消失**、log 出現「面板收起 … 手動結束」
19. 結束後再下一次 `/horntail` → 開出新面板，正常運作
20. 非 GM／非管理員按「結束面板」→ 被拒絕，**面板不受影響**
21. log 檢查：每一次「面板建立」後面，最終都要有一筆對應的「面板收起」及其原因
...
**先講排查結果，這件事的原因不在我們的程式。**

翻 8/22、8/23 的 log，測試站與正式站**同時**出現大量：

```
Connect Timeout Error (162.159.x.x:443, timeout: 10000ms)
unable to verify the first certificate
shardError(shard 0)：UNABLE_TO_VERIFY_LEAF_SIGNATURE
```

8/23 21:58～22:08 這十分鐘內，測試站 690 筆 shardError ＋ 702 次重連，
正式站 644 筆——**兩個 jail 一起發作，等於是 jail 對外的網路或 TLS 出問題，不是 Discord 限流**。
使用者感受到的「計時器延遲」正好落在這兩個時段。

程式端救不了網路，但可以讓**畫面不再依賴網路**：

| | 改之前 | 改之後 |
|---|---|---|
| 倒數怎麼來 | bot 每 2 秒編輯訊息重寫數字 | Discord 相對時間戳 `<t:秒:R>`，**看的人自己的用戶端在算** |
| 每分鐘編輯次數 | 約 30 次 | 約 4 次（只在有人按按鈕、或某一輪歸零時） |
| 網路不穩時 | 畫面卡住，秒數停在舊值 | **秒數照跑**，因為根本不需要 bot |

代價是**按鈕標籤放不了秒數**（Discord 只在訊息內容與 embed 裡算時間戳），
所以按鈕只剩招式名與「（計時中）」。秒數在 embed 裡，後面附一個 `<t:秒:T>` 的絕對結束時刻——
相對時間超過一分鐘只會顯示「2 分鐘後」，不夠精確，補上時刻才看得出確切秒數。

### 09-18　提醒改回剩 5 秒

原本設 7 秒，是為了補「<發訊者> 說」這段 TTS 開場約兩秒。
但實際使用時**發訊者的名稱 TTS 唸不出來**，那段開場等於不存在，於是每次都早響兩秒。
`WARN_SECONDS` 改回 5。

`WARN_DISPLAY_SECONDS` 保留（現在兩個都是 5），這樣之後若又需要補償開場，
只要改 `WARN_SECONDS`、面板文字不動。

### 09-19　再按一次是「重新計時」，不是「停止」

打王遇到延遲或遊戲本身 delay 時，使用者要的是**從頭重算**。
舊語意（在跑就停）得按兩次才做得到，而每按一次都要等一輪 Discord 互動往返，現場來不及。

`timerState.toggleTimer()` 改名為 `pressTimer()`，行為固定是 `startTimer()`——
沒在跑就開始，正在跑就從初始秒數重來（`warned` 也一起重置，這一輪的提醒會重新算）。
`timerService.toggleSkill()` 同步改名 `pressSkill()`。

**副作用（已知並接受）：單一招式沒有辦法個別停止了**，要停只能按「全部停止」。
如果之後發現常常需要只停一招，再考慮長按或第二排按鈕，但那會讓面板變複雜。

**09-13 的驗收（2026-08-21 已實作，尚未驗收）**：

22. 語音頻道的人全部離開 → **60 秒後**面板自動收起，log 出現「面板收起 … 語音頻道沒人」
23. 有人在 60 秒內回到語音頻道 → 面板**不會**被收掉，計時繼續
24. **計時器正在跑的時候**所有人離開 → 一樣會收（這是唯一一個跑著也會收的條件）
25. 只剩 bot 在語音頻道 → 算沒人，會收

**09-17～09-19 的驗收（2026-08-24，尚未驗收）**：

26. 按下招式 → embed 顯示「N 秒後（時:分:秒）」，**數字自己在跳**（那是你的用戶端在算）
27. 把 bot 停掉（`pm2 stop discord-bot`）→ 畫面上的倒數**仍然繼續跳**
28. 計時中再按同一顆 → **秒數回到滿值繼續跑**，不會停下來
29. 剩 5 秒 → TTS 響（不再是 7 秒）
30. 一輪歸零 → embed 的時間戳換成下一輪，按鈕維持「（計時中）」
31. 跑滿 10 分鐘後看 log → **`編輯偏慢` 與 `編輯面板失敗` 都是 0**，且整段期間的編輯次數遠少於改版前
...
- 2026-08-21　**管理員一律放行**（09-11）。理由：管理員本來就能自己加 GM 身分組，擋他只是多繞一圈。fail closed 的語意不變——沒設定 `permissionRoles.gm` 時仍然只有管理員能用。
- 2026-08-21　**`default_member_permissions` 改成 `0`**（09-12）。程式端只能做到「預設不給看」，要讓 GM 也看得到必須**在伺服器設定裡手動加覆寫**，那一步改程式改不到（需要 OAuth2 使用者授權，bot token 沒有這個權限）。
- 2026-08-21　**只能在語音頻道的聊天室使用**（09-14）。理由：TTS 是由「正在看該頻道」的人的用戶端朗讀的，開在一般文字頻道等於提醒不存在。判斷用 `channel.isVoiceBased()`，語音頻道內建的文字聊天，頻道本身就是那個語音頻道。
- 2026-08-21　**「結束面板」與「全部停止」分成兩顆按鈕**（09-15）。理由：打王中途常要全部暫停一下，一顆按鈕若會收掉面板，暫停一次就得重下指令。
- 2026-08-21　**每一次 `closePanel()` 都寫 log 並帶原因**（09-16）。理由：原本只有「面板建立」有紀錄，被新面板取代那條路徑完全沒有輸出，查「那個面板收掉了沒」時沒有依據。
- 2026-08-21　**`src/main.js` 加 `GuildVoiceStates` intent**（09-13）。使用者同意動這個檔案。少了它 `channel.members` 永遠是空的，面板會被誤收。非特權 intent，不必去開發者後台開啟。
- 2026-08-21　**語音頻道沒人的寬限定為 60 秒**（`VOICE_EMPTY_MS`）。理由：斷線重連是常事，設 0 的話打到一半面板就沒了。
- 2026-08-24　**倒數改用 `<t:秒:R>` 由用戶端計算**（09-17）。理由：8/22、8/23 兩次網路事件（連線逾時 ＋ TLS 驗證失敗，兩個 jail 同時發作）讓畫面整個卡住；改成用戶端算之後，bot 連不上也不影響倒數，順帶把編輯次數從每分鐘 30 次降到約 4 次。代價是按鈕標籤放不了秒數。
- 2026-08-24　**提醒改回剩 5 秒**（09-18）。理由：原本提早兩秒是補 TTS 的「<發訊者> 說」開場，但發訊者名稱唸不出來，那段開場不存在。
- 2026-08-24　**再按一次＝重新計時，不是停止**（09-19）。理由：現場遇到 delay 要的是重算，舊語意得按兩次、等兩輪互動往返。代價是單一招式無法個別停止，只能用「全部停止」。
- 2026-08-21　**「語音頻道沒人」是唯一一個計時器跑著也會收的條件**。理由：人都走光了，倒數給誰看；另外兩個條件（總時限、閒置）都會避開正在跑的面板。
