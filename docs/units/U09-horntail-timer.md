...
狀態：進行中
進度：15/16（原定 10 步已實機驗收通過；09-11～09-16 是驗收後追加，只剩 09-13 卡在 `main.js` 的 intent）
...
- [ ] 09-13 綁定的語音頻道沒人時自動收面板（**擋住：需要 `src/main.js` 加 `GuildVoiceStates` intent**，等使用者同意）
...
> 2026-08-21 補充：09-14 上線後，面板一定開在語音頻道的聊天室，
> 所以「綁哪個頻道」不必再靠 `ctx.member.voice.channel` 去猜——
> **面板所在的頻道本身就是那個語音頻道**，`panel.channelId` 直接就是要監看的對象。
> 上表「沒在語音頻道就不綁」那一列也不再需要。這讓 09-13 變簡單了，但 intent 仍然是硬前提。
...
22. 語音頻道的人全部離開 → 過寬限時間後面板自動收起，log 有紀錄
23. 有人斷線後在寬限時間內回來 → 面板**不會**被收掉
