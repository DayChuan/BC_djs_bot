...
| 環境 | 日文頻道 ID | 老師身分組 |
|---|---|---|
| 測試站（伺服器 `974484668252565544`） | `1539905791564193862` | **未設定**（使用者指示測試站先不測此功能） |
...
2. `/jp post:true` 立刻在日文頻道發一則（測試站沒有老師身分組，這一步驗不了，見阻擋項）
3. 重啟 bot → 開機 log 出現「排程已註冊：japanese:daily」，且已發過的紀錄還在，不會從頭再發一次
4. `/jp_history range:本月` 列得出剛才那幾則
5. `/jp_note` 加一則筆記，再 `/jp` 抽到同一筆時看得到

---

## 阻擋項

**測試站沒有老師身分組**（2026-08-25，使用者指示「測試站台先不測此功能」）。因為權限刻意**不放行管理員**，測試站的 `/jp_admin` 與 `/jp post:true` 會是誰都不能用的狀態，**這兩塊在測試站驗收不到，只能直接在正式站驗**。要在測試站驗的話，需要使用者在測試伺服器建一個身分組並把 id 填進 `test.js` 的 `permissionRoles.teacher`。
