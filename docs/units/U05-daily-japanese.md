...
- 2026-08-25　`parseImport` 配號時要 `while(used.has(...)) next += 1`。第一次 jail 跑測試就抓到：`next` 只在有改號時前進，但本批內保留原號的那幾筆一樣占用號碼，所以「一次貼兩筆都叫 `j_0001`」時第二筆會又被配到 `j_0001`。資料表非空時剛好看不出來（`next` 從最大號 +1 起算），別把這個測試刪掉。
