...
上線後使用者回報：**一般成員在鎖定的討論串裡，按鈕是灰的、按不下去，投票完全不能用。**
...
| 解除鎖定後 | 正常 |
| 討論串有沒有被封存 | 沒有 |
| bot 的 log | **一筆錯誤都沒有** —— 互動根本沒送出來 |

**原本文件寫的「鎖定只擋發訊息，不擋元件互動」是錯的。** 唯讀頻道裡的按鈕面板
（例如 `/role-panel`）能用，那是**頻道權限**；討論串的 `locked` 是另一回事，它會直接停用元件。

當初的驗收第 5 條有列這件事，但**很可能是用管理員帳號驗的**——管理員有 `ManageThreads`，
在鎖定的討論串裡照樣能發言，按鈕當然是亮的。**這類權限相關的驗收一律要用一般成員帳號。**

### 為什麼沒有別的解法

- **討論串沒有自己的權限覆寫。** discord.js 的 `ThreadChannel` 只有 `permissionsFor()`（查詢），
  沒有 `permissionOverwrites`（設定）；Discord 的 UI 也找不到，因為那個功能不存在，一律繼承母頻道
- 改**母頻道**的「在討論串中發送訊息」會讓該頻道底下**所有**討論串都被禁言，
  而使用者的頻道還有很多其他用途的討論串，不能這樣做

### 現在的作法

**不鎖定。** 討論串本身就把投票跟主頻道的聊天隔開了，這是它最主要的價值；
就算有人在串裡聊幾句，也只影響那一串。

剩下唯一「一串一設」又不會停用按鈕的工具是**慢速模式**（`rateLimitPerUser`，上限 6 小時），
已經接好但**預設關閉**：要用就把 `src/config/polls.js` 的 `THREAD_SLOWMODE_SECONDS` 改成秒數。

## 決策紀錄

- 2026-09-08　**移除 `setLocked(true)`**。理由：實測鎖定會讓一般成員的按鈕變灰按不下去，投票不能用，而 bot 端看不到任何錯誤（互動沒送出來）。
- 2026-09-08　不改母頻道權限。理由：討論串沒有自己的權限覆寫，改母頻道會波及該頻道底下所有其他用途的討論串。
- 2026-09-08　慢速模式接好但預設 0（關閉）。理由：它是唯一「一串一設」又不停用元件的工具，但要不要用是使用者的取捨，不該替他決定。
