...
### 4-C `/quickpoll` 語音快速投票

| 項目 | 內容 |
|---|---|
| 指令 | `/quickpoll question: choices:2/3/4 [minutes:]`，一般成員可用 |
| 顏色 | 全部使用 Discord 內建按鈕配色，不掛 emoji。2＝紅藍、3＝紅藍灰、4＝紅藍灰綠（**沒有黃色按鈕**） |
| 呈現 | 訊息公開且即時更新：每個顏色的長條、票數與百分比 |
| 投票 | 單選。再按一次同一個顏色＝取消（現場常按錯，沒有取消就只能重開一場） |
| 結束 | 「結束投票」按鈕，限發起人或管理員；或到時間自動結束（預設 30 分鐘） |
| 結算 | 就地把原訊息換成最終結果並移除按鈕，**不另外發一則** —— 它本來就即時顯示，再洗一則只是噪音 |
| 資料 | 與一般投票同一套 store，`type: 'quick'`，一樣會歸檔並受 90 天保留期限管理 |

**一定要有自動結束時間**：沒有的話，沒人按結束就會一直留在進行中的清單裡，
而語音現場的快速投票通常沒有人會記得回來收尾。

**Progress**
- [x] 4-C-1 `pollRender`：顏色定義與即時比數訊息
...
一人多角色**必須搭配身分群組**。沒有身分可以區分的話，面板上只會是一堆
「第 N 筆」，結算名單也無從分組。所以：

- `/poll` 在建立階段就擋下來（開 `multi_char` 卻沒選 `identity` → 直接拒絕並說明）
- 面板端另有 `canMultiChar()` 防護，改版前留下的舊資料不會出現沒有意義的多角色介面
- **不提供事後切換** —— 排程投票如果當初沒開，砍掉重開即可，由建立的人在一開始注意
...
## 測試範圍的取捨（2026-08-18 決議）

**只為資料層與純邏輯寫測試，不為介面寫測試。**

| 寫 | 不寫 |
|---|---|
| `pollStore`、`pollArchive`、`scheduler`、`pollIdentities` | `pollRender`、`pollAdmin` 的按鈕排列與面板長相 |
| 併發不覆寫、時區換算、統計、保留期限、檔案格式、舊格式相容 | customId 的組成、元件有幾列、按鈕文字 |
| 寫一次就不太需要動 | 介面一改就要整批重寫 |

**理由**：介面測試的維護成本極高（這幾天每次調整介面都要重寫一輪），
而實際抓到的 bug 是零；反而發生過「把錯誤行為寫成預期」的情況
