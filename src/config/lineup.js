...
//要出幾份名單。取票數最高的層級，同一層級(同票數)的日期全部列出；
//**湊到這個數量就停手，不再往下取第二高票**。
//
//例：票數 5、5、3 → 最高票就有兩個日期，這樣就夠了，3 票那天不列。
//    票數 5、3、3 → 最高票只有一個，往下取 3 票那個層級(兩個日期全列)，共三份。
//使用者 2026-09-03 確認：第一名同票有兩個時就只取那兩個。
export const TOP_OPTION_COUNT = 2

//最多往下取幾個票數層級。防止「5、1、1、1、1」這種一路往下撈的情形。
